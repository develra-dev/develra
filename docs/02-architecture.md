...
The original product direction proposes reusing the founder's existing monitoring stack:
...
That is a reasonable future default, but none of it belongs in the first local scanner. The shared contract is the normalized lockfile, provider IDs, operation IDs, and registry interface.
